...
const Planets: React.FC<PlanetData> = ({
    data 
})=> {
  const [favs, setFavs] =useState([{1:"hi"}, {2:"bye"}]);
  const handleAddToFavs = (data:any) =>{
    setFavs(favs=>[...favs,data]);
  }
  ...
  return (
    <Box m={2} border={1} width={800} className={classes.flexContainer}>
      ...
      <Box m={2} border={1} width={100} height={200}>
          ...
          <Button variant="contained" onClick={()=>{handleAddToFavs(data.name)}}>Save</Button>
      </Box>
    </Box>
  );
}
